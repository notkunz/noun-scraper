...
async function findTMALinks(page, roundNumber) {
  ...
  const courseLinks = await page.evaluate(() => {
    ...
    return Array.from(document.querySelectorAll('a[href*="/course/view"]'))
      ...
      .filter(l => {
        if (!l.text || seen.has(l.href)) return false
        seen.add(l.href)
        return true
      })
  })
...
async function scrapeQuestions(page, quizHref) {
  await page.goto(quizHref, { waitUntil: 'domcontentloaded', timeout: 30000 })
  ...
  while (hasNext) {
    const pqs = await page.evaluate((si) => {
      return Array.from(document.querySelectorAll('.que')).map((el, idx) => {
        ...
        clone.querySelectorAll('.answer, .outcome, .comment, .gradingdetails, input, button').forEach(e => e.remove())
  ...
  return { courseCode, questions }
}

// Original scrape endpoint (for extension use)
app.post('/scrape-tma', async (req, res) => {
  ...
  try {
    ...
    for (const quiz of quizLinks) {
      try {
        const { courseCode, questions } = await scrapeQuestions(page, quiz.href)
...
async function answerQuestion(question, options, materialCode, courseId, userId) {
  ...
  if (courseId) {
    const { data: bank } = await supabase
      ...
      .limit(1).single()
    if (bank) return { answer: bank.answer_text, source: 'question_bank', bankId: bank.id }
  }

  // Search material chunks using sliding window
  const words = question.replace(/[^a-zA-Z\s]/g, ' ').split(' ').filter(w => w.length > 2)
  const phrases = [...words]
...
2. For fill-in-blank find the exact sentence with those words completed
3. Match to the closest option
4. Reply ONLY with the letter and option text e.g "B. Success"
5. If not found in material reply: ANSWER_NOT_FOUND`
    }],
    max_tokens: 256
  })
...
/**/
  ...
  try {
    ...
    // Check if this matric + tmaRound was already run before
    ...
    const isRerun = !!previousRun

    if (!isRerun) {
      // First time — deduct token
    ...
    for (const quiz of quizLinks) {
      try {
        const { courseCode, questions } = await scrapeQuestions(page, quiz.href)
        await log(runId, `${courseCode}: ${questions.length} questions found`)
        ...
        for (const q of questions.slice(0, 10)) {
          try {
            const { answer, source } = await answerQuestion(
              q.questionText, q.options, materialCode, course?.id, userId
            )
            ...
            allResults.push({
              courseCode,
              courseTitle: quiz.text.replace(/Course is starred.*?(?=\w)/s, '').trim(),
            ...
            allResults.push({
              courseCode, courseTitle: quiz.text,
